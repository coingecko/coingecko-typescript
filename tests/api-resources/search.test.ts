...
describe('resource search', () => {
  ...
  test.skip('retrieve: only required params', async () => {
    const responsePromise = client.search.retrieve({ query: 'query' });
  ...
  test.skip('retrieve: required and optional params', async () => {
    const response = await client.search.retrieve({ query: 'query' });
  });
  ...
  test.skip('listTrending: request options and params are passed correctly', async () => {
    ...
    await expect(
      client.search.listTrending({ show_max: 'show_max' }, { path: '/_stainless_unknown_path' }),
