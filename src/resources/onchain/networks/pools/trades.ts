...
export class Trades extends APIResource {
  ...
  /**
   * To query the trades within a range of timestamp based on the provided pool
   * address
   */
  getRange(
    poolAddress: string,
    ...
    const { network, ...query } = params;
    return this._client.get(path`/onchain/networks/${network}/pools/${poolAddress}/trades/range`, {
      query,
      ...options,
    });
  }
